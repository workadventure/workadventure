...
test.describe('Availability Status', () => {
    test.describe('Busy Status', () => {
        test('should not return to online status when you move', async({page, browser, browserName}, {project}) => {
            ...
            await expect(page.getByText("Busy")).toHaveCSS('opacity','0.5')
        ...
        test.describe('busy interaction',async()=>{
            ...
            test('should open a popup when a user approaches', async({page, browserName, browser, context}, {project})=>{
                if (browserName === "webkit") {
                    //eslint-disable-next-line playwright/no-skipped-test
                    test.skip();
                    return;
            ...
            test('should be in proximity meeting after accepting conversation',async({ page, browser,context,browserName}, {project})=>{
                if(browserName === "webkit"){
                    //eslint-disable-next-line playwright/no-skipped-test
                    test.skip();
                    return;
    ...
    test.describe('Do not disturb Status',()=>{
        test('should not return to online status when you move',async({ page, browser,browserName },{project})=>{
            ...
            await expect(page.getByText("Do not disturb")).toHaveCSS('opacity','0.5')
