...
test.describe("Availability Status", () => {
    test.describe("Busy Status", () => {
        test("should return to online status when you move @nowebkit", async ({ browser }, { project }) => {
        ...
        test.describe("busy interaction", () => {
            test("should open a popup when a bubble is created... @nowebkit", async ({ browserName, browser }) => {
                test.skip(browserName === "webkit", "WebKit limitations");
                const statusName = "Busy";
                await using page = await getPage(
                    browser,
