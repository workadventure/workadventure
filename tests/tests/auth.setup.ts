...
const aliceStorageTest = './.auth/Alice.json'
const bobStorageTest = './.auth/Bob.json'
//const John = './.auth/John.json'
const characterNumber = 3;

setup('login as Alice', async ({ page }) => {
    await page.goto(
    ...
    await page.context().storageState({ path: aliceStorageTest});
...
setup('login as Bob', async ({ page }) => {
    ...
    //login
    await page.fill('input[name="loginSceneName"]', "Bob");
    await page.click('button.loginSceneFormSubmit');
    await expect(page.locator('button.selectCharacterSceneFormSubmit')).toBeVisible();
    for (let i = 0; i < characterNumber; i++) {
        await page.keyboard.press('ArrowRight');
    ...
    await page.context().storageState({ path: bobStorageTest})
});

/*setup('login as John', async ({ page }) => {
    await page.goto(
        publicTestMapUrl(`tests/RemotePlayers/remote_players.json`, "api-players")
    );
    
    //login
    await page.fill('input[name="loginSceneName"]', "John");
    await page.click('button.loginSceneFormSubmit');
    await expect(page.locator('button.selectCharacterSceneFormSubmit')).toBeVisible();
    for (let i = 0; i < characterNumber; i++) {
        await page.keyboard.press('ArrowRight');
