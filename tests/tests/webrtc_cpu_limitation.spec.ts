...
// The cells of the box run together in textContent ("Codec:VP9/VP9Source:P2P (2 encoders)Limited by:cpu").
...
test.describe("CPU-limited encoders @nomobile", () => {
    ...
    test("the bubble moves to LiveKit on the member's flag and stays there", async ({ browser }) => {
        ...
        await expect.poll(() => boxText(alice), { timeout: 2 * WINDOW_MS }).toMatch(/Codec:\s*H264/i);
    ...
    test("without a back that acts on the flag, the member renegotiates a cheaper codec with both peers", async ({
        ...
        await expect.poll(() => boxText(alice), { timeout: 2 * WINDOW_MS }).toMatch(/Codec:\s*H264/i);
        await expect.poll(() => boxText(bob), { timeout: 30_000 }).toMatch(/Codec:\s*H264/i);
        await expect.poll(() => boxText(carol), { timeout: 30_000 }).toMatch(/Codec:\s*H264/i);
