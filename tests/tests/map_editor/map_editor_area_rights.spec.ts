...
test.describe("Map editor area with rights @oidc @nomobile @nowebkit", () => {
    ...
    test("Access restricted area with right click to move", async ({ browser, request }) => {
        ...
        const actualPositionAfterRightClickToMove = await evaluateScript(page, async () => {
            // eslint-disable-next-line
            // @ts-ignore
            return await WA.player.getPosition();
        });
    ...
    test("Area with restricted write access : Trying to remove an object outside the area", async ({
        ...
        // Add area with admin rights
        await Menu.openMapEditor(page);
        await AreaAccessRights.openAreaEditorAndAddAreaWithRights(page, ["admin"], []);
        await AreaAccessRights.openEntityEditorAndAddEntityWithOpenLinkPropertyOutsideArea(page);
        await oidcLogout(page);
    ...
    test("Claim multi personal area", async ({ browser, request }) => {
        ...
        // Add a second area
        await Menu.openMapEditor(page);
        await AreaAccessRights.openAreaEditorAndAddArea(page, { x: 1 * 32, y: 10 * 32 }, { x: 9 * 32, y: 19 * 32 });
        await page.getByTestId("personalAreaPropertyData").click();
        await Menu.closeMapEditor(page);
