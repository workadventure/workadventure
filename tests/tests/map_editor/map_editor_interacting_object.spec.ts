...
test.describe("Map editor interacting with object @oidc @nomobile", () => {
    ...
    test("Success to interact with area @nowebkit", async ({ browser, request }) => {
        ...
        // The interaction hint is rendered as a Phaser DOMElement attached to the player.
        await expect(
            page.locator("span.characterTriggerAction").filter({ hasText: "to open web site 👀" }),
        ).toBeVisible();
    });
    ...
    test("Success to interact with entity @nowebkit", async ({ browser, request, browserName }) => {
        ...
        // Open new page
        await using newPage = await getPage(browser, "User1", Map.url("empty"));
