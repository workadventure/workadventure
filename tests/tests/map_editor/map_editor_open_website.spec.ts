...
import { map_storage_url, maps_test_url } from "../utils/urls.js";
...
test.describe("Map editor @oidc @nomobile @nowebkit", () => {
    ...
    test("Successfully set open website area in the map editor, with working Scripting API @nofirefox", async ({
        browser,
        request,
    }) => {
    ...
    test("Successfully set GoogleWorkspace's applications in the area in the map editor", async ({
        ...
        const googleDockButtonLocator = page
        ...
        await googleDockButtonLocator.fill(
        ...
        await expect(googleDriveButtonLocator).toHaveValue(
            "https://drive.google.com/file/d/1DjNjZVbVeQO9EvgONLzCtl6wG-kxSr9Z/preview",
            { timeout: 10_000 },
        );
