...
test.describe("Map editor @oidc @nomobile @nowebkit", () => {
    ...
    test('Successfully set "SpeakerZone" with see attendees option in the map editor', async ({ browser, request }) => {
       ...
       test("Megaphone auditorium mode with 5 participants (LiveKit stress test)", async ({ browser, request }) => {
        ...
        for (const { page } of listeners) {
            await expect(page.locator("#cameras-container").getByText('Admin1', { exact: true })).toBeVisible({ timeout: 20_000 });
        }

        // In auditorium mode, the speaker should see all listeners
        for (const { name } of listeners) {
            await expect(pageSpeaker.locator("#cameras-container").getByText(name, { exact: true })).toBeVisible({ timeout: 20_000 });
        }

        // Verify listeners can see each other in auditorium mode
        // Admin2 (listener1) should see Alice, Bob, and John
        await expect(pageListener1.locator("#cameras-container").getByText('Alice', { exact: true })).toBeHidden({ timeout: 20_000 });
        await expect(pageListener1.locator("#cameras-container").getByText('Bob', { exact: true })).toBeHidden({ timeout: 20_000 });
        await expect(pageListener1.locator("#cameras-container").getByText('John', { exact: true })).toBeHidden({ timeout: 20_000 });

        // Alice (listener2) should see Admin2, Bob, and John
        await expect(pageListener2.locator("#cameras-container").getByText('Admin2', { exact: true })).toBeHidden({ timeout: 20_000 });
        await expect(pageListener2.locator("#cameras-container").getByText('Bob', { exact: true })).toBeHidden({ timeout: 20_000 });
        await expect(pageListener2.locator("#cameras-container").getByText('John', { exact: true })).toBeHidden({ timeout: 20_000 });
