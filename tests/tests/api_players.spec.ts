...
import { getCoWebsiteIframe } from "./utils/iframe.ts";
...
test.describe("API WA.players @nomobile @nowebkit", () => {
    ...
    test("that player B arriving after player A set his variables can read the variable.", async ({ browser }) => {
    ...
    const runPersistenceTest = async (page: Page, browser: Browser) => {
      ...
      if (WA === undefined) {
        ...
        expect(await readRemotePlayerVariable("Alice", "non_public_persisted", page2)).toBe(undefined);
        expect(await readRemotePlayerVariable("Alice", "non_public_non_persisted", page2)).toBe(undefined);
        await expect
            .poll(() => readRemotePlayerVariable("Alice", "public_non_persisted", page2))
            .toBe("public_non_persisted");
        await expect.poll(() => readRemotePlayerVariable("Alice", "public_persisted", page2)).toBe("public_persisted");
        await expect
            .poll(() => readRemotePlayerVariable("Alice", "public_persisted_json", page2))
    ...
    test("that 2 players sharing the same UUID are notified of persisted private variable changes.", async ({
        browser,
    }) => {
        await using page = await getPage(browser, "Alice", publicTestMapUrl("tests/E2E/empty.json", "api_players"));
