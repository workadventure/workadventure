...
class CoWebsiteManager {
    ...
    constructor() {
        this.cowebsiteDiv = HtmlUtils.getElementByIdOrFail<HTMLDivElement>(
            cowebsiteDivId
        );
    }
    ...
    public loadCoWebsite(url: string, allowPolicy?: string): void {
        ...
        setTimeout(() => {
            HtmlUtils.getElementByIdOrFail("cowebsite-close").addEventListener(
                "click",
                () => {
                    this.closeCoWebsite();
                }
     ...
     */
    public insertCoWebsite(
        callback: (cowebsite: HTMLDivElement) => Promise<void>
    ): void {
        this.load();
        this.currentOperationPromise = this.currentOperationPromise
            .then(() => callback(this.cowebsiteDiv))
            .then(() => {
                this.open();
                setTimeout(() => {
                    this.fire();
