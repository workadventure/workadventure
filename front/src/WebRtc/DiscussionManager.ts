...
export class DiscussionManager {
    ...
    private participants: Map<number | string, HTMLDivElement> = new Map<
        number | string,
        HTMLDivElement
    >();
    ...
    private sendMessageCallBack: Map<
        number | string,
        SendMessageCallback
    > = new Map<number | string, SendMessageCallback>();

    private userInputManager?: UserInputManager;

    constructor() {
        this.mainContainer = HtmlUtils.getElementByIdOrFail<HTMLDivElement>(
            "main-container"
        );
    ...
    private createDiscussPart(name: string) {
        ...
        const buttonCloseDiscussion: HTMLButtonElement = document.createElement(
            "button"
        );
    ...
    public addParticipant(
        ...
        const divImgParticipant: HTMLImageElement = document.createElement(
            "img"
        );
        ...
        const divPParticipant: HTMLParagraphElement = document.createElement(
            "p"
        );
        if (!name) {
            name = "Anonymous";
        ...
        if (
            ...
            const reportBanUserAction: HTMLButtonElement = document.createElement(
                "button"
            );
    ...
    public addMessage(name: string, message: string, isMe: boolean = false) {
        ...
        setTimeout(() => {
            this.divMessages?.scroll({
                top:
                    this.divMessages?.scrollTop +
                    divMessage.getBoundingClientRect().y,
                behavior: "smooth",
            });
        }, 200);
    }
