...
export type ShowReportCallBack = (
    userId: string,
...
export class MediaManager {
    ...
    private remoteVideo: Map<string, HTMLVideoElement> = new Map<
        string,
        HTMLVideoElement
    >();
    ...
    private triggerCloseJistiFrame: Map<String, Function> = new Map<
        String,
        Function
    >();

    constructor() {
        this.myCamVideo = HtmlUtils.getElementByIdOrFail<HTMLVideoElement>(
            "myCamVideo"
        );
        this.webrtcInAudio = HtmlUtils.getElementByIdOrFail<HTMLAudioElement>(
            "audio-webrtc-in"
        );
        this.webrtcOutAudio = HtmlUtils.getElementByIdOrFail<HTMLAudioElement>(
            "audio-webrtc-out"
        );
        this.webrtcInAudio.volume = 0.2;
        this.webrtcOutAudio.volume = 0.2;

        this.microphoneBtn = HtmlUtils.getElementByIdOrFail<HTMLDivElement>(
            "btn-micro"
        );
        this.microphoneClose = HtmlUtils.getElementByIdOrFail<HTMLImageElement>(
            "microphone-close"
        );
        ...
        this.microphone = HtmlUtils.getElementByIdOrFail<HTMLImageElement>(
            "microphone"
        );
        ...
        this.cinemaBtn = HtmlUtils.getElementByIdOrFail<HTMLDivElement>(
            "btn-video"
        );
        this.cinemaClose = HtmlUtils.getElementByIdOrFail<HTMLImageElement>(
            "cinema-close"
        );
        ...
        this.cinema = HtmlUtils.getElementByIdOrFail<HTMLImageElement>(
            "cinema"
        );
        ...
        this.monitorBtn = HtmlUtils.getElementByIdOrFail<HTMLDivElement>(
            "btn-monitor"
        );
        this.monitorClose = HtmlUtils.getElementByIdOrFail<HTMLImageElement>(
            "monitor-close"
        );
        ...
        this.monitor = HtmlUtils.getElementByIdOrFail<HTMLImageElement>(
            "monitor"
        );
        ...
        this.previousConstraint = JSON.parse(
            JSON.stringify(this.constraintsMedia)
        );
    ...
    removeUpdateLocalStreamEventListener(
        callback: UpdatedLocalStreamCallback
    ): void {
        this.updatedLocalStreamCallBacks.delete(callback);
    }

    private triggerUpdatedLocalStreamCallbacks(
    ...
    public showGameOverlay() {
        ...
        const buttonCloseFrame = HtmlUtils.getElementByIdOrFail(
            "cowebsite-close"
        );
        ...
        this.getCamera()
            .then((stream: MediaStream) => {
                ...
                if (stream.getVideoTracks().length === 0) {
                    throw Error(
                        "Video track is empty, please check camera permission of your navigator"
                    );
                }
                this.enableCameraStyle();
                this.triggerUpdatedLocalStreamCallbacks(stream);
        ...
        this.getCamera()
            .then((stream) => {
                ...
                if (stream.getAudioTracks().length === 0) {
                    throw Error(
                        "Audio track is empty, please check microphone permission of your navigator"
                    );
                }
    ...
    private disableScreenSharing() {
        ...
        if (this.localScreenCapture === null) {
            console.warn(
                "Weird: trying to remove a screen sharing that is not enabled"
            );
            return;
        }
        ...
        try {
            return this._startScreenCapture()
                .then((stream: MediaStream) => {
                    ...
                    this.addScreenSharingActiveVideo(
                        "me",
                        DivImportance.Normal
                    );
                    HtmlUtils.getElementByIdOrFail<HTMLVideoElement>(
                        "screen-sharing-me"
                    ).srcObject = stream;
    ...
    async getCamera(): Promise<MediaStream> {
        if (navigator.mediaDevices === undefined) {
            if (window.location.protocol === "http:") {
                throw new Error(
                    "Unable to access your camera or microphone. You need to use a HTTPS connection."
                );
            } else {
                throw new Error(
                    "Unable to access your camera or microphone. Your browser is too old."
                );
            }
        }
        ...
        return this.getLocalStream().catch(() => {
            ...
            return this.getLocalStream()
                ...
                .catch((err) => {
                    console.info(
                        "error get media ",
                        this.constraintsMedia.video,
                        this.constraintsMedia.audio,
                        err
                    );
                    throw err;
                });
    ...
    private getLocalStream(): Promise<MediaStream> {
        return navigator.mediaDevices
            .getUserMedia(this.constraintsMedia)
            .then((stream: MediaStream) => {
    ...
    addActiveVideo(
        ...
        this.remoteVideo.set(
            userId,
            HtmlUtils.getElementByIdOrFail<HTMLVideoElement>(userId)
        );
        ...
        if (!anonymous) {
            const reportBanUserAction: HTMLImageElement = HtmlUtils.getElementByIdOrFail<HTMLImageElement>(
                `report-${userId}`
            );
    ...
    addScreenSharingActiveVideo(
        userId: string,
        divImportance: DivImportance = DivImportance.Important
    ) {
    ...
    isError(userId: string): void {
        ...
        const errorDiv = element
            .getElementsByClassName("rtc-error")
    ...
    private getSpinner(userId: string): HTMLDivElement | null {
        ...
        const connnectingSpinnerDiv = element
    ...
    public addNewParticipant(
        ...
        img?: string,
        showReportUserCallBack?: ShowReportCallBack
    ) {
        discussionManager.addParticipant(
            userId,
            name,
            img,
            false,
            showReportUserCallBack
        );
    }
