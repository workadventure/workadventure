...
class LayoutManager {
    ...
    private importantDivs: Map<string, HTMLDivElement> = new Map<
        string,
        HTMLDivElement
    >();
    private normalDivs: Map<string, HTMLDivElement> = new Map<
        string,
        HTMLDivElement
    >();
    private listener: CenterListener | null = null;

    private actionButtonTrigger: Map<string, Function> = new Map<
        string,
        Function
    >();
    private actionButtonInformation: Map<string, HTMLDivElement> = new Map<
        string,
        HTMLDivElement
    >();
    ...
    public add(importance: DivImportance, userId: string, html: string): void {
        ...
        if (importance === DivImportance.Important) {
            ...
            if (
                this.importantDivs.size === 1 &&
                this.mode === LayoutMode.VideoChat
            ) {
    ...
    private positionDiv(elem: HTMLDivElement, importance: DivImportance): void {
        if (this.mode === LayoutMode.VideoChat) {
            const chatModeDiv = HtmlUtils.getElementByIdOrFail<HTMLDivElement>(
                "chat-mode"
            );
            ...
            if (importance === DivImportance.Important) {
                ...
                const sideBarDiv = HtmlUtils.getElementByIdOrFail<HTMLDivElement>(
                    "sidebar"
                );
    ...
    private removeFocusOn(userId: string): void {
        ...
        if (importantDiv === undefined) {
            throw new Error(
                'Div with user id "' + userId + '" is not in important mode'
            );
        }
     ...
     */
    public findBiggestAvailableArray(): {
        xStart: number;
        yStart: number;
        xEnd: number;
        yEnd: number;
    } {
        const game = HtmlUtils.querySelectorOrFail<HTMLCanvasElement>(
            "#game canvas"
        );
        if (this.mode === LayoutMode.VideoChat) {
            const children = document.querySelectorAll<HTMLDivElement>(
                "div.chat-mode > div"
            );
            ...
            const area1 =
                (game.offsetWidth -
                    (lastDiv.offsetLeft + lastDiv.offsetWidth)) *
            ...
            const area2 =
                game.offsetWidth *
                (game.offsetHeight -
                    (lastDiv.offsetTop + lastDiv.offsetHeight));
            ...
            const mainSectionChildren = Array.from(
                document
                    .querySelectorAll<HTMLDivElement>("div.main-section > div")
                    .values()
            );
            const sidebarChildren = Array.from(
                document
                    .querySelectorAll<HTMLDivElement>("aside.sidebar > div")
                    .values()
            );
            ...
            const lastPresentationDiv =
                mainSectionChildren[mainSectionChildren.length - 1];

            const presentationArea =
                (game.offsetHeight -
                    (lastPresentationDiv.offsetTop +
                        lastPresentationDiv.offsetHeight)) *
                (lastPresentationDiv.offsetLeft +
                    lastPresentationDiv.offsetWidth);

            let leftSideBar: number;
            let bottomSideBar: number;
            if (sidebarChildren.length === 0) {
                leftSideBar = HtmlUtils.getElementByIdOrFail<HTMLDivElement>(
                    "sidebar"
                ).offsetLeft;
                bottomSideBar = 0;
            } else {
                const lastSideBarChildren =
                    sidebarChildren[sidebarChildren.length - 1];
                leftSideBar = lastSideBarChildren.offsetLeft;
                bottomSideBar =
                    lastSideBarChildren.offsetTop +
                    lastSideBarChildren.offsetHeight;
            }
            const sideBarArea =
                (game.offsetWidth - leftSideBar) *
            ...
            if (presentationArea <= sideBarArea) {
                ...
                return {
                    ...
                    yStart:
                        ...
                        lastPresentationDiv.offsetHeight,
                    xEnd:
                        /*lastPresentationDiv.offsetLeft + lastPresentationDiv.offsetWidth*/ game.offsetWidth, // To avoid flickering when a chat start, we center on the center of the screen, not the center of the main content area
                    yEnd: game.offsetHeight,
                };
            }
        }
    ...
    public addActionButton(
        ...
        const mainContainer = HtmlUtils.getElementByIdOrFail<HTMLDivElement>(
            "main-container"
        );
