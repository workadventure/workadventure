...
export const loadAllLayers = (
    ...
    LAYERS.forEach((layer) => {
        ...
        Object.values(layer).forEach((textureDescriptor) => {
            ...
            load.spritesheet(textureDescriptor.name, textureDescriptor.img, {
                frameWidth: 32,
                frameHeight: 32,
            });
        });
...
export const loadAllDefaultModels = (
    ...
    returnArray.forEach((playerResource: BodyResourceDescriptionInterface) => {
        load.spritesheet(playerResource.name, playerResource.img, {
            frameWidth: 32,
            frameHeight: 32,
        });
    });
...
export const loadCustomTexture = (
    ...
    const playerResourceDescriptor: BodyResourceDescriptionInterface = {
        name,
        img: texture.url,
        level: texture.level,
    };
...
export const lazyLoadPlayerCharacterTextures = (
    ...
    texturekeys.forEach(
        (textureKey: string | BodyResourceDescriptionInterface) => {
            try {
                //TODO refactor
                const playerResourceDescriptor = getRessourceDescriptor(
                    textureKey
                );
                if (
                    playerResourceDescriptor &&
                    !loadPlugin.textureManager.exists(
    ...
    let returnPromise: Promise<
        Array<string | BodyResourceDescriptionInterface>
    >;
...
export const getRessourceDescriptor = (
    ...
    const textureName: string =
        typeof textureKey === "string" ? textureKey : textureKey.name;
...
const createLoadingPromise = (
    ...
    return new Promise<BodyResourceDescriptionInterface>((res) => {
        ...
        loadPlugin.once(
            "filecomplete-spritesheet-" + playerResourceDescriptor.name,
