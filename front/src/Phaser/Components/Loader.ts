...
export const addLoader = (scene: Phaser.Scene): void => {
    ...
    const promiseLoadLogoTexture = new Promise<Phaser.GameObjects.Image>(
        (res) => {
            if (scene.load.textureManager.exists(LogoNameIndex)) {
                return res(
                    scene.add.image(
                        scene.game.renderer.width / 2,
                        scene.game.renderer.height / 2 - 150,
                        LogoNameIndex
                    )
                );
            } else {
                //add loading if logo image is not ready
                loadingText = scene.add.text(
                    scene.game.renderer.width / 2,
    ...
    scene.load.on("complete", () => {
        ...
        promiseLoadLogoTexture.then(
            (resLoadingImage: Phaser.GameObjects.Image) => {
