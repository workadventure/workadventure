...
export class GameScene extends ResizableScene implements CenterListener {
    ...
    MapPlayersByKey: Map<number, RemotePlayer> = new Map<
        number,
        RemotePlayer
    >();
    ...
    private async onMapLoad(data: any): Promise<void> {
        ...
        for (const [itemType, objectsOfType] of objects) {
            ...
            switch (itemType) {
                ...
                default:
                    throw new Error(
                        'Unsupported object type: "' + itemType + '"'
                    );
            }
            ...
            this.load.on("complete", () => {
                ...
                this.createPromise.then(async () => {
                    ...
                    for (const object of objectsOfType) {
                        ...
                        const actionableItem = itemFactory.factory(
                            this,
                            object,
                            state
                        );
                        this.actionableItems.set(
                            actionableItem.getId(),
                            actionableItem
                        );
                    }
                });
            ...
            .then((onConnect: OnConnectInterface) => {
                ...
                this.connection.onUserMoved((message: UserMovedMessage) => {
                    ...
                    if (position === undefined) {
                        throw new Error(
                            "Position missing from UserMovedMessage"
                        );
                    }

                    const messageUserMoved: MessageUserMovedInterface = {
                        userId: message.getUserid(),
                        position: ProtobufClientUtils.toPointInterface(
                            position
                        ),
                    };
                ...
                this.connection.onServerDisconnected(() => {
                    console.log(
                        "Player disconnected from server. Reloading scene."
                    );
                    ...
                    const gameSceneKey =
                        "somekey" + Math.round(Math.random() * 10000);
                ...
                this.GlobalMessageManager = new GlobalMessageManager(
                    this.connection
                );
                this.UserMessageManager = new UserMessageManager(
                    this.connection
                );
                ...
                if (newValue === undefined) {
                    ...
                    const openWebsiteFunction = () => {
                        coWebsiteManager.loadCoWebsite(
                            newValue as string,
                            allProps.get("openWebsitePolicy") as
                                | string
                                | undefined
                        );
                        layoutManager.removeActionButton(
                    ...
                    if (
                        ...
                        layoutManager.addActionButton(
                            "openWebsite",
                            "Click on SPACE to open the web site",
                            () => {
                                openWebsiteFunction();
                            },
                            this.userInputManager
                ...
                if (newValue === undefined) {
                    ...
                    const openJitsiRoomFunction = () => {
                        if (JITSI_PRIVATE_MODE) {
                            const adminTag = allProps.get(
                                "jitsiRoomAdminTag"
                            ) as string | undefined;

                            this.connection.emitQueryJitsiJwtMessage(
                    ...
                    const jitsiTriggerValue = allProps.get(
                        TRIGGER_JITSI_PROPERTIES
                    );
                    if (
                        jitsiTriggerValue &&
                        ...
                        layoutManager.addActionButton(
                            "jitsiRoom",
                            "Click on SPACE to enter in jitsi meet room",
                            () => {
    ...
    private onMapExit(exitKey: string) {
        ...
        if (!roomId)
            throw new Error(
                "Could not find the room from its exit key: " + exitKey
            );
    ...
    private initPositionFromLayerName(layerName: string) {
        for (const layer of this.mapFile.layers) {
            if (
                ...
                (layerName === defaultStartLayerName ||
                    this.isStartLayer(layer))
            ) {
    ...
    private outlineItem(event: HasMovedEvent): void {
        ...
        switch (event.direction) {
            ...
            default:
                throw new Error(
                    'Unexpected direction "' + event.direction + '"'
                );
        }
    ...
    update(time: number, delta: number): void {
        ...
        updatedPlayersPositions.forEach(
            (moveEvent: HasMovedEvent, userId: number) => {
                const player:
                    | RemotePlayer
                    | undefined = this.MapPlayersByKey.get(userId);
                if (player === undefined) {
                    throw new Error(
                        'Cannot find player with ID "' + userId + '"'
                    );
                }
    ...
    private doShareGroupPosition(
        ...
        const sprite = new Sprite(
            ...
            groupPositionMessage.groupSize === 4
                ? "circleSprite-red"
                : "circleSprite-white"
        );
    ...
    emitActionableEvent(
        ...
        this.connection.emitActionableEvent(
            itemId,
            eventName,
            state,
            parameters
