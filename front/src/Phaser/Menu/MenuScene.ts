...
export class MenuScene extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.html(
            gameSettingsMenuKey,
            "resources/html/gameQualityMenu.html"
        );
    ...
    create() {
        this.menuElement = this.add
            .dom(closedSideMenuX, 30)
            .createFromCache(gameMenuKey);
        this.menuElement.setOrigin(0);
        this.revealMenusAfterInit(this.menuElement, "gameMenu");

        const middleX = window.innerWidth / 3 - 298;
        this.gameQualityMenuElement = this.add
            .dom(middleX, -400)
            .createFromCache(gameSettingsMenuKey);
        this.revealMenusAfterInit(this.gameQualityMenuElement, "gameQuality");

        this.gameShareElement = this.add
            .dom(middleX, -400)
            .createFromCache(gameShare);
        ...
        this.gameShareElement.on("click", (event: MouseEvent) => {
            ...
            if (
                (event?.target as HTMLInputElement).id === "gameShareFormSubmit"
            ) {
                this.copyLink();
            } else if (
                (event?.target as HTMLInputElement).id === "gameShareFormCancel"
            ) {
                this.closeGameShare();
            }
        });

        this.gameReportElement = this.add
            .dom(middleX, -400)
            .createFromCache(gameReport);
        ...
        this.gameReportElement.on("click", (event: MouseEvent) => {
            ...
            if (
                ...
                "gameReportFormSubmit"
            ) {
                this.submitReport();
            } else if (
                (event?.target as HTMLInputElement).id ===
                "gameReportFormCancel"
            ) {
                this.closeGameReport();
            }
        });
        mediaManager.setShowReportModalCallBacks(
            this.openGameReport.bind(this)
        );
    ...
    private revealMenusAfterInit(
        menuElement: Phaser.GameObjects.DOMElement,
        rootDomId: string
    ) {
    ...
    public revealMenuIcon(): void {
        (this.menuButton.getChildByID(
            "menuIcon"
        ) as HTMLElement).hidden = false;
    }
    ...
    openSideMenu() {
        ...
        if (
            gameManager.getCurrentGameScene(this).connection &&
            gameManager.getCurrentGameScene(this).connection.isAdmin()
        ) {
            const adminSection = this.menuElement.getChildByID(
                "adminConsoleSection"
            ) as HTMLElement;
    ...
    private closeSideMenu(): void {
        ...
        this.menuButton.getChildByID(
            "openMenuButton"
        ).innerHTML = `<img src="/static/images/menu.svg">`;
        gameManager
            .getCurrentGameScene(this)
            .ConsoleGlobalMessageManager.disabledMessageConsole();
        this.tweens.add({
            targets: this.menuElement,
            x: closedSideMenuX,
            duration: 500,
    ...
    private openGameSettingsMenu(): void {
        ...
        this.gameQualityMenuElement.on("click", (event: MouseEvent) => {
            ...
            if (
                ...
                this.saveSetting(
                    parseInt(gameQualitySelect.value),
                    parseInt(videoQualitySelect.value)
                );
            } else if (
                (event?.target as HTMLInputElement).id ===
                "gameQualityFormCancel"
            ) {
                this.closeGameQualityMenu();
            }
        });
    ...
    private openGameShare(): void {
        ...
        const gameShareLink = this.gameShareElement.getChildByID(
            "gameShareLink"
        ) as HTMLInputElement;
    ...
    private closeGameShare(): void {
        const gameShareInfo = this.gameShareElement.getChildByID(
            "gameShareInfo"
    ...
    private openGameReport(userId: string, userName: string | undefined) {
        ...
        const gameTitleReport = this.gameReportElement.getChildByID(
            "nameReported"
        ) as HTMLElement;
        gameTitleReport.innerText = userName
            ? `Report user: ${userName}`
            : "Report user";
        const gameIdUserReported = this.gameReportElement.getChildByID(
            "idUserReported"
        ) as HTMLInputElement;
        ...
        gameManager
            .getCurrentGameScene(this)
            .userInputManager.clearAllInputKeyboard();
    ...
    private closeGameReport(): void {
        ...
        gameManager
            .getCurrentGameScene(this)
            .userInputManager.initKeyBoardEvent();
        this.tweens.add({
            targets: this.gameReportElement,
            y: -400,
            duration: 1000,
    ...
    private submitReport(): void {
        const gamePError = this.gameReportElement.getChildByID(
            "gameReportErr"
        ) as HTMLParagraphElement;
        gamePError.innerText = "";
        gamePError.style.display = "none";
        const gameTextArea = this.gameReportElement.getChildByID(
            "gameReportInput"
        ) as HTMLInputElement;
        const gameIdUserReported = this.gameReportElement.getChildByID(
            "idUserReported"
    ...
    private onMenuClick(event: MouseEvent) {
        if (
            (event?.target as HTMLInputElement).classList.contains("not-button")
        ) {
            return;
        }
        event.preventDefault();
        ...
        switch ((event?.target as HTMLInputElement).id) {
            ...
            case "adminConsoleButton":
                gameManager
                    .getCurrentGameScene(this)
                    .ConsoleGlobalMessageManager.activeMessageConsole();
                break;
        }
    }
    ...
    private gotToCreateMapPage() {
        const sparkHost =
            "https://" +
            window.location.host.replace("play.", "") +
            "/choose-map.html";
