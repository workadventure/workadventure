...
<div class="report-menu-main
  ...
  <section>
    <button type="button" class="close" on:click|preventDefault={close}>&times</button>
  ...
  <section class="report-menu-action {disableReport ? 'tw-hidden' : ''}">
    <section class="tw-flex tw-justify-center">
      <button
        type="button"
        class="{blockActive ? 'disabled' : 'light'}"
    ...
    <section class="tw-flex tw-justify-center">
      <button
        type="button"
        class="{reportActive ? 'disabled' : 'light'}"
