...
export class ConsoleGlobalMessageManager {
    ...
    initialise() {
        ...
        textMessage.addEventListener("click", () => {
            textMessage.classList.add("active");
            const messageSection = HtmlUtils.getElementByIdOrFail<HTMLInputElement>(
                this.getSectionId(INPUT_CONSOLE_MESSAGE)
            );
            messageSection.classList.add("active");

            textAudio.classList.remove("active");
            const audioSection = HtmlUtils.getElementByIdOrFail<HTMLInputElement>(
                this.getSectionId(UPLOAD_CONSOLE_MESSAGE)
            );
    ...
    sendMessage() {
        const inputType = HtmlUtils.getElementByIdOrFail<HTMLInputElement>(
            INPUT_TYPE_CONSOLE
        );
        if (
            AUDIO_TYPE !== inputType.value &&
    ...
    private async sendAudioMessage() {
        const inputAudio = HtmlUtils.getElementByIdOrFail<HTMLInputElement>(
            UPLOAD_CONSOLE_MESSAGE
        );
