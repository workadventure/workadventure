...
export class GlobalMessageManager {
    ...
    initialise() {
        ...
        this.Connection.receivePlayGlobalMessage(
            (message: PlayGlobalMessageInterface) => {
    ...
    private playMessage(message: PlayGlobalMessageInterface) {
        const previousMessage = document.getElementById(
            this.getHtmlMessageId(message.id)
        );
        if (previousMessage) {
    ...
    private playAudioMessage(messageId: string, urlMessage: string) {
        ...
        const previousDivAudio = document.getElementsByClassName(
            "audio-playing"
        );
        ...
        const messageAudioSource: HTMLSourceElement = document.createElement(
            "source"
        );
    ...
    private stopMessage(messageId: string) {
        HtmlUtils.removeElementByIdOrFail<HTMLDivElement>(
            this.getHtmlMessageId(messageId)
        );
    }
