...
export class Room {
    ...
    public async getMapUrl(): Promise<string> {
        return new Promise<string>((resolve, reject) => {
            ...
            if (this.isPublic) {
                ...
                if (!match)
                    throw new Error(
                        'Could not extract url from "' + this.id + '"'
                    );
    ...
    public getInstance(): string {
        ...
        if (this.isPublic) {
            ...
            if (!match)
                throw new Error(
                    'Could not extract instance from "' + this.id + '"'
                );
    ...
    private parsePrivateUrl(
        url: string
    ): { organizationSlug: string; worldSlug: string; roomSlug?: string } {
        const regex = /@\/([^/]+)\/([^/]+)(?:\/([^/]*))?/gm;
