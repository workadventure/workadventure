...
class LocalUserStore {
    ...
    setCustomCursorPosition(activeRow: number, selectedLayers: number[]): void {
        window.localStorage.setItem(
            ...
            JSON.stringify({ activeRow, selectedLayers })
        );
    }
    getCustomCursorPosition(): {
        activeRow: number;
        selectedLayers: number[];
    } | null {
        return JSON.parse(
            window.localStorage.getItem("customCursorPosition") || "null"
        );
    }
    ...
    getCharacterLayers(): string[] | null {
        return JSON.parse(
            window.localStorage.getItem(characterLayersKey) || "null"
        );
    }

    getGameQualityValue(): number {
        return (
            parseInt(window.localStorage.getItem(gameQualityKey) || "") || 60
        );
    }
    ...
    getVideoQualityValue(): number {
        return (
            parseInt(window.localStorage.getItem(videoQualityKey) || "") || 20
        );
    }
