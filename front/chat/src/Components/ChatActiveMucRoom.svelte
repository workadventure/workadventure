<script lang="ts">
  ...
  export let activeMucRoom: MucRoom;
  export let settingsView = false;

  const usersListStore = activeMucRoom.getPresenceStore();
  const meStore = activeMucRoom.getMeStore();
...
>
  <div class="wa-thread-head">
    ...
    <div class="tw-text-center tw-pt-1 tw-pb-2">
      <div class="tw-flex">
        <b>{activeMucRoom.name}</b>
        {#if activeMucRoom.type === "live"}
      ...
      >
        {#if $meStore.isAdmin}
          <button
                  class="wa-action"
                  type="button"
                  on:click|stopPropagation={() => activeMucRoom.reInitialize()}
  ...
  {:else}
    <ChatMessagesList room={activeMucRoom} bind:this={messagesList} />
