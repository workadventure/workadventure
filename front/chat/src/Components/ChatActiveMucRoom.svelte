...
<!-- thread -->
    ...
    {:else if $settingsViewStore}
        ...
        >
            ...
            >
                {#if $meStore.isAdmin}
                    <button class="wa-action" type="button" on:click|stopPropagation={() => mucRoom.reInitialize()}
    ...
    {:else}
        <ChatMessagesList mucRoom={mucRoom} bind:this={messagesList} />

        <div class="messageForm">
            <ChatMessageForm mucRoom={mucRoom} on:scrollDown={messagesList.scrollDown} />
        </div>
    {/if}
