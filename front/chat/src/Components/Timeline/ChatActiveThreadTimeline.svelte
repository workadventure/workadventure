<script lang="ts">
  ...
  function writing() {
    if (newMessageText != undefined && newMessageText !== "") {
      ...
      _newChatMessageWritingStatusSubject.next(
        ChatMessageTypes.userStopWriting
      );
    }
  }
  ...
  function handlerKeyUp(keyUpEvent: KeyboardEvent) {
    if (
      ...
      (keyUpEvent.key === "Enter" || keyUpEvent.code === "Enter")
    ) {
      saveMessage();
      return;
    }
    ...
    >
      ...
      >
        <button
          class="wa-action"
          type="button"
          on:click|stopPropagation={reInitialize}
        >
      ...
      {#each $chatMessagesStore as message}
        {#if message.type === ChatMessageTypes.text || message.type === ChatMessageTypes.me}
          <div class="tw-mt-2">
            <div
              class={`tw-flex ${
                message.type === ChatMessageTypes.me
                  ? "tw-justify-end"
                  : "tw-justify-start"
              }`}
            >
              <div
                class={`${
                  message.type === ChatMessageTypes.me || !message.author
                    ? "tw-opacity-0"
                    : "tw-mt-4"
