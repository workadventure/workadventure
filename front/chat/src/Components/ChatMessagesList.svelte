<script lang="ts">
  ...
  import {AlertCircleIcon, Trash2Icon, RefreshCwIcon} from "svelte-feather-icons";
        ...
        <div class="tw-flex tw-flex-row tw-items-center">
          <div class={`tw-flex ${isMe(message.name) ? "tw-justify-end" : "tw-justify-start"}`}>
            ...
            >
              ...
              <div class={`tw-rounded-lg tw-bg-dark tw-text-xs tw-px-3 tw-py-2 ${isMe(message.name)?'tw-text-right':'tw-text-left'}`}>
                <p class="tw-mb-0 tw-whitespace-pre-line">{message.body}</p>
              </div>
            </div>
          </div>
          {#if message.error}
            <div class="wa-error-message" on:mouseleave={() => document.getElementById(`error_${message.id}`).classList.add('tw-invisible')}>
            <div
                    class={`tw-text-pop-red tw-ml-1 tw-flex ${needHideHeader(message.name, message.time, i)?'':'tw-mt-4'}`}
            ...
            >
              <span class="wa-dropdown-item" on:click={() => mucRoom.sendBack(message.id) && document.getElementById(`error_${message.id}`).classList.add('tw-invisible')}>
                <RefreshCwIcon size="13" class="tw-mr-1"/> {$LL.sendBack()}
              </span>
              <span class="wa-dropdown-item tw-text-pop-red" on:click={() => mucRoom.deleteMessage(message.id) && document.getElementById(`error_${message.id}`).classList.add('tw-invisible')}>
                <Trash2Icon size="13" class="tw-mr-1"/> {$LL.delete()}
              </span>
            </div>
            </div>
          {/if}
