...
<div class={`wa-chat-item`} on:mouseleave={closeChatUserMenu}>
  ...
  <div
    class="tw-flex-auto tw-ml-2"
    on:click|stopPropagation={() => open(mucRoom)}
  >
    <h1 class="tw-text-sm tw-font-bold tw-mb-0">
      {#each chunks as chunk (chunk.key)}
        <span class={`${chunk.match ? "tw-text-light-blue" : ""}`}
          >{chunk.text}</span
        >
      {/each}
    </h1>
  ...
  <div class="wa-dropdown">
    ...
    <!-- menu -->
    <div
      class={`wa-dropdown-menu ${forumMenuActive ? "" : "tw-invisible"}`}
    ...
    >
      ...
      {#if $meStore.isAdmin && mucRoom.type === "live"}
        <span
          class="wa-dropdown-item tw-text-pop-red"
          on:click|stopPropagation={() => mucRoom.reInitialize()}
