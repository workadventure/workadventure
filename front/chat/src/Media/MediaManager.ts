    ...
    if (this.hasNotification()) {
      ...
      switch (notificationType) {
        case NotificationType.discussion:
          new Notification(
            `${userName} ${get(LL).notification.discussion()}`,
            options
          );
          break;
    ...
    if (this.canPlayNotificationMessage && elementAudioNewMessageNotification) {
      ...
      (elementAudioNewMessageNotification as HTMLAudioElement)
        ...
        .then(() => {
          ...
          return setTimeout(
            () => (this.canPlayNotificationMessage = true),
            TIME_NOTIFYING_MILLISECOND
          );
        })
        .catch((err) =>
          console.error("Trying to play notification message error: ", err)
        );
    }
  }
}
