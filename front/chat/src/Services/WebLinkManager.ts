...
export class WebLink {
  ...
  private cardRenderer(
    ...
    if (description != undefined) {
      ...
      descriptionElement.append(
        HtmlUtils.htmlDecode(description.substring(0, 100) + "...")
      );
  ...
  private getHyperLinkHTMLElement(
    withEmbedly: boolean = true
  ): HTMLAnchorElement {
    const link = document.createElement("a");
    link.href = this.link;
    link.target = "_blank";
  ...
  get contentWebsiteRenderer(): Promise<string> {
    return (async () => {
      if (EMBEDLY_KEY != undefined && EMBEDLY_KEY !== "") {
        try {
          const url = `https://api.embedly.com/1/oembed?url=${encodeURI(
            this.link
          )}&key=${EMBEDLY_KEY}`;
          ...
          if (data.html) {
            ...
            if (
              extension != undefined &&
              FileMessageManager.isImage(extension)
            ) {
              return (
                this.getHyperLinkHTMLElement(false).outerHTML +
                this.imageRendererHtml(uuidv4(), data.provider_name)
            ...
            if (
              extension != undefined &&
              FileMessageManager.isVideo(extension)
            ) {
              return (
                this.getHyperLinkHTMLElement(false).outerHTML +
            ...
            if (
              extension != undefined &&
              FileMessageManager.isSound(extension)
            ) {
              return (
                this.getHyperLinkHTMLElement(false).outerHTML +
