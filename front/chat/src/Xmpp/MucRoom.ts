...
export class MucRoom {
  ...
  public getPlayerName() {
    try {
      return (
        connectionManager.connectionOrFail.getXmppClient()?.getPlayerName() ??
        "unknown"
      );
    } catch (e) {
      console.log(e);
  ...
  private sendPresence() {
    ...
    if (!this.closed) {
      ...
      if (_VERBOSE)
        console.warn("[XMPP]", ">> Presence sent", get(userStore).uuid);
    }
  }
  ...
  private sendSubscribe() {
    ...
    if (!this.closed) {
      ...
      if (_VERBOSE)
        console.warn("[XMPP]", ">> Subscribe sent", this.roomJid.local);
    }
  }
  ...
  public sendMessage(text: string, messageReply?: Message) {
    ...
    if (messageReply != undefined) {
      ...
      if (messageReply.files != undefined) {
        xmlReplyMessage.append(
          fileMessageManager.getXmlFileAttrFrom(messageReply.files)
        );
      }
    ...
    if (!this.closed) {
      ...
      this.messageStore.update((messages) => {
        messages.push({
          ...
          from: this.jid,
          type:
            messageReply != undefined ? messageType.reply : messageType.message,
  ...
  onMessage(xml: ElementExt): void {
    ...
    // We are receiving the presence from someone
        ...
        if (!state) {
          ...
          if (xml.getChild("reaction") != undefined) {
            ...
            this.messageReactStore.update((reactMessages) => {
              ...
              //check if already exist
              if (
                !newReactMessages.find(
                  (react) => react.id === newReactMessage.id
                )
              ) {
                newReactMessages.push(newReactMessage);
                reactMessages.set(newReactMessage.message, newReactMessages);
            ...
            this.messageStore.update((messages) => {
              if (messages.find((message) => message.id === idMessage)) {
                ...
                const owner = [
                  ...get(mucRoomsStore.getDefaultRoom().getPresenceStore()),
      ...
      xml.getName() === "message" &&
      xml.getChild("result", "urn:xmpp:mam:2")
    ) {
      ...
      const state = messageXML?.getChildByAttr(
        "xmlns",
        "http://jabber.org/protocol/chatstates"
      );
      let delay = xml
      ...
      if (!state && delay) {
        ...
        const message: Message = {
          ...
          error: false,
          from: from.toString(),
          type: messageXML?.getChild("reply")
            ? messageType.message
            : messageType.reply,
        };
