...
describe("CommunicationManager", () => {
    ...
    describe("session analytics", () => {
        ...
        it("reports the space whose users reach neither registry, once the clients have had time to watch", async () => {
            // The guard on the assumption this whole derivation rests on: everyone in the
            // space watches it. Nothing enforces that, and its failure is otherwise silent.
            // Asserted through console.error rather than Sentry, whose ESM export cannot be
            // spied on — the guard writes to both.
            vi.useFakeTimers();
            const consoleError = vi.spyOn(console, "error").mockImplementation(() => {});
            try {
                await withQueue(async () => {
                    const space = createSpace([createSpaceUser("ghost"), createSpaceUser("seen")]);
                    space.getMetadataValue = vi.fn().mockReturnValue("bubble");
                    const manager = managerFor(space);

                    await manager.handleUserToNotifyAdded(createSpaceUser("seen"));

                    // Not on the spot: a listener is in the space before their client has
                    // watched it, and that round trip must not be reported as a break.
                    expect(consoleError).not.toHaveBeenCalled();

                    await vi.advanceTimersByTimeAsync(30_000);
                    expect(consoleError).toHaveBeenCalledOnce();
                    expect(String(consoleError.mock.calls[0][0])).toContain("neither registry");
                    expect(consoleError.mock.calls[0][1]).toEqual(["ghost"]);

                    // Once per space: a later arrival does not report it again.
                    await manager.handleUserToNotifyAdded(createSpaceUser("third"));
                    await vi.advanceTimersByTimeAsync(30_000);
                    expect(consoleError).toHaveBeenCalledOnce();
                });
            } finally {
        ...
        it("says nothing when the stragglers catch up before the check", async () => {
            vi.useFakeTimers();
            const consoleError = vi.spyOn(console, "error").mockImplementation(() => {});
            try {
                await withQueue(async () => {
                    const listener = createSpaceUser("listener");
                    const space = createSpace([listener, createSpaceUser("seen")]);
                    space.getMetadataValue = vi.fn().mockReturnValue("bubble");
                    const manager = managerFor(space);

                    await manager.handleUserToNotifyAdded(createSpaceUser("seen"));
                    await vi.advanceTimersByTimeAsync(1_000);
                    await manager.handleUserToNotifyAdded(listener);
