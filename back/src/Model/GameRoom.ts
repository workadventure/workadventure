...
export class GameRoom {
    ...
    public static async create(
        ...
        const mapDetails = await GameRoom.getMapDetails(roomUrl, userId);
     ...
     */
    private static async getMapDetails(roomUrl: string, userId?: string): Promise<MapDetailsData> {
        if (!ADMIN_API_URL) {
        ...
        const result = await adminApi.fetchMapDetails(roomUrl, userId);
