...
import type { CreatePollQuery, SpaceUser } from "@workadventure/messages";
...
export class ProximityPollManager {
    ...
    public create(sender: SpaceUser, { question, kind, answers, maxSelections }: CreatePollQuery): void {
    ...
    /** An empty `answerIds` withdraws the sender's vote. */
    public vote(sender: SpaceUser, pollId: string, answerIds: string[]): void {
    ...
    public close(sender: SpaceUser, pollId: string, closingMessage?: string): void {
        const poll = this.getPoll(pollId);
        ...
        this.space.updateState((state) => {
            state.polls[pollId].end =
                closingMessage === undefined ? { closedAt: Date.now() } : { closingMessage, closedAt: Date.now() };
        });
    }

    public delete(sender: SpaceUser, pollId: string): void {
