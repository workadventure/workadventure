...
const api: WorkAdventurePipApi = {
    onOffer: (callback) => subscribe("app:pip:offer", (sdp) => callback(sdp as PipSdp)),
    onIce: (callback) => subscribe("app:pip:ice-from-main", (candidate) => callback(candidate as RTCIceCandidateInit)),
    ...
    identifyScreens: () => ipcRenderer.invoke("app:pip:identify-screens"),
