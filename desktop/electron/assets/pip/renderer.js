...
(function () {
    ...
    function renderSourcePicker() {
        ...
        if (filtered.length === 0) {
            if (lastSources.length === 0 && sourcePickerKind === "screen") {
                showPermissionHint();
                return;
            }
            sourcePickerBody.className = "sp-body empty";
            sourcePickerBody.textContent = sourcePickerKind === "screen" ? "No screen available." : "No window available.";
            return;
        }
        ...
        filtered.forEach(function (source) {
            ...
            tile.addEventListener("click", function () {
                api.sendCommand({ type: "pick-source", sourceId: source.id, sourceName: source.name || "" });
                closeSourcePicker();
            });
