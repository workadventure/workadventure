...
(async () => {
    connect('BetaTester').catch(e => console.error(e));
...
async function connect(username: string): Promise<Agent> {
    ...
    client.on("session:started", async () => {
        ...
        const recipient = JID.create({
            ...
            local: "http://play.workadventure.localhost/_/global/maps.workadventure.localhost/starter/map.json/forum/welcome",
        ...
        for (let i = 0; i < 200; i++) {
            ...
            client.sendMessage({
                type: "groupchat",
                to: "http\\3a\\2f\\2fplay.workadventure.localhost\\2f_\\2fglobal\\2fmaps.workadventure.localhost\\2fstarter\\2fmap.json\\2fforum\\2fwelcome@conference.ejabberd",
                id: messageId,
                jid: myJid,
                body: "test message n°"+i
            });
            messagesId.push(messageId);
            let endTime = performance.now();
            console.log(`[${Math.trunc((endTime - start) / 1000)}s][${username}][${Math.trunc(endTime - startTime)}ms] Message n°${i} sent.`);

            await wait(300);
        }

        for (const id of messagesId) {
            client.sendGroupChatMessage({
                to: "http\\3a\\2f\\2fplay.workadventure.localhost\\2f_\\2fglobal\\2fmaps.workadventure.localhost\\2fstarter\\2fmap.json\\2fforum\\2fwelcome@conference.ejabberd",
                id: uuid(),
                jid: myJid,
                body: "",
