...
<div class="file">
    {#if FileMessageManager.isImage(url)}
        <img src={url} width="100%" height="100%" alt={name ?? FileMessageManager.getName(url)} class="mt-2" />
    {:else if FileMessageManager.isVideo(url)}
        <video width="100%" height="100%" alt={name ?? FileMessageManager.getName(url)} class="mt-2" controls>
    ...
    {:else if FileMessageManager.isSound(url)}
        <audio width="100%" height="100%" controls class="mt-2" src={url}>
            Your browser does not support the <code>audio</code> element.
        </audio>
    {:else}
        <div class="other-content" on:click={download}>
            <div class="icon p-0 mr-1">
                <FileIcon size="22" />
            </div>
            <div class="information">
                <p class="m-0">{name ?? FileMessageManager.getName(url)}</p>
                <!--
                <p class="text-light-purple-alt mt-1 m-0 text-xxs">
