<script lang="ts">
    ...
    export let shownRoomList: string;
    ...
    $: usersLenght = [...$presenceStore.values()].filter((user: User) => {
        ...
        <div class="users tw-border-b tw-border-solid tw-border-0 tw-border-transparent tw-border-b-light-purple">
            <div class="tw-px-4 tw-py-1 tw-flex tw-items-center">
                ...
                <button
                    ...
                    on:click={() => shownRoomListStore.set($shownRoomList === room ? "" : room)}
                >
                    <ChevronUpIcon
                        class={`tw-transform tw-transition ${$shownRoomList === room ? "" : "tw-rotate-180"}`}
                    />
                </button>
            </div>
            {#if $shownRoomList === room}
