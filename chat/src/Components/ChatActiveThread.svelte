...
>
    ...
    {:else}
        ...
        {/if}
        <div
            class:tw-hidden={$usersListViewStore}
            in:fly={{ y: 100, duration: 100, delay: 200 }}
            bind:this={messagesList}
        >
            <ChatMessagesList {activeThread} bind:this={messagesList} />

            <div class="messageForm" transition:fly={{ y: 100, duration: 100 }}>
                <ChatMessageForm {activeThread} on:scrollDown={messagesList.scrollDown} />
            </div>
        </div>
    {/if}
</div>
