...
<div class="first-connection">
    {#if $step === 1}
        ...
        <p class="tw-text-justify">
            We use Matrix as the protocol for our chat.
        </p>
        <p class="tw-text-justify">
        ...
        <button class="btn light tw-font-bold tw-mx-auto tw-cursor-pointer" on:click={() => step.set(2)}>Next step</button>
    {:else}
        {#if !$finished}
            ...
            {#if $loading}
                Loading
            {:else}
                <button class="btn light tw-font-bold tw-mx-auto tw-cursor-pointer" disabled={!passwordChecked} on:click={submit}>Submit</button>
            {/if}
        {/if}
