 ...
 */
export const jitsiDeviceOptionsStore = derived([requestedJitsiDevicesStore, requestedCameraState, requestedMicrophoneState], ([$requestedJitsiDevicesStore, $requestedCameraState, $requestedMicrophoneState]) => {
    if ($requestedJitsiDevicesStore === undefined) {
        return undefined;
    }
    const devices = structuredClone($requestedJitsiDevicesStore) as Set<DeviceType>;
...
export const jitsiLocalTracksStore = derived<Readable<CreateLocalTracksOptions | undefined>, Result<JitsiLocalTracks, Error> | undefined>(jitsiDeviceOptionsStore, ($jitsiDeviceOptionsStore, set) => {
    ...
    JitsiMeetJS.createLocalTracks($jitsiDeviceOptionsStore).then((newTracks) => {
