...
/**
 * Meetings held somewhere else: a Teams or Google Meet area.
 *
 ...
 * Nobody observes that tab: a user who walks away with Teams still open leaves our
 ...
 */
const PROVIDERS: Record<string, "teams" | "google_meet"> = {
    teams: "teams",
    google: "google_meet",
};

/** One live meeting per module subtype: you cannot be in two Teams areas at once. */
...
export function externalMeetingStarted(subtype: string, areaId: string, roomId: string): void {
    const meetingProvider = PROVIDERS[subtype];
    if (meetingProvider === undefined) {
        // A module that declares a meeting area under a name we have no provider for.
        // Reporting it with no provider would put an unattributable row in the meeting
        // series; saying nothing leaves the generic area.dwell, which still measures it.
        return;
    }
    ...
    analyticsClient.trackAdminEvent("meeting.area_entered", { roomId, meetingProvider });
