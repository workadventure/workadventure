    ...
    <div class="w-2/3 relative h-fit">
        <PopUpContainer extraClasses="w-full relative">
            ...
            >
                ...
                {:else}
                    <!-- room card -->
                    <div class="h-fit w-full flex flex-col items-center gap-4">
                        <div class="w-full">
                            <label for="search" class="w-1/2 text-white pb-2 pl-4 block"
                        ...
                        <div class="flex flex-row flex-nowrap space-x-4 justify-center items-start w-full">
                            {#if $isFetching}
                            ...
                            {:else}
                                <div class="relative w-full flex space-x-6 snap-mandatory snap-x overflow-x-auto">
                                    ...
                                    {#each Array.from($roomListFiltered) as [roomUrl, roomData] (roomUrl)}
                                        ...
                                        >
                                            <div class="w-full rounded-lg relative overflow-hidden group cursor-pointer">
                                                <div class="absolute z-10  left-0 bottom-0-0 w-full h-full bg-gradient-to-t to-90% to-transparent {currentRoomUrl === new URL(roomData.roomUrl, window.location.href).toString()?'from-secondary-900' :'from-contrast'}"></div>
                                                <img
                                                ...
                                                />
                                                <span class="absolute z-20 bottom-0 max-w-full w-full flex items-center justify-center text-ellipsis overflow-hidden text-white text-xs font-bold px-2 py-3 bg-se">
                                                    {roomData.name}
                                                </span>
                                                {#if currentRoomUrl === new URL(roomData.roomUrl, window.location.href).toString()}
                                                    <span class="chip z-20 chip-sm chip-secondary bg-secondary text-white rounded-[8px] absolute top-3 right-3">
                                                        <div class="px-2">
                                                            Active
                                                        </div>
                                                    </span>
