...
>
    <img alt="Sub menu arrow"  loading="eager" src={tooltipArrow} class="content-[''] absolute -top-1 left-9 m-auto w-2 h-1" />
    <div class="relative z-10 pb-4 rounded-lg overflow-hidden">
        {#if hasImage}
            <img alt="Help GIF image" src={image} class="w-full -mt-[2px]" loading="lazy" />
        {/if}
