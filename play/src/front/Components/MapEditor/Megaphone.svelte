<script lang="ts">
    ...
    let scopes = [{value: "WORLD",label: $LL.mapEditor.settings.megaphone.inputs.world()},{value:"ROOM",label: $LL.mapEditor.settings.megaphone.inputs.room()}];
    ...
    async function getTags() {
        ...
        loading = false;
        return _tags.filter((item, index) => _tags.indexOf(item) === index).map(tag => ({value: tag, label: tag.toLocaleUpperCase()}));
    }
</script>


<div class="tw-flex tw-flex-wrap tw-gap-x-4 tw-items-center tw-h-fit">
    <input type="checkbox" class="input-switch" bind:checked={enabled} on:change={partialSave} disabled={loading ? "disabled": ""}/>
...
{#if enabled}
    <div class="settings tw-flex-grow tw-flex-auto tw-flex-shrink" transition:fade={{ duration: 200 }}>
        ...
        {:then tags}
            <InputText errorHelperText={dynamicStrings.error.title} label={$LL.mapEditor.settings.megaphone.inputs.spaceName()} placeHolder="MySpace" bind:value={title} onKeyPress={() => dynamicStrings.error.title = ""} onFocus={onMapEditorInputFocus} onBlur={onMapEditorInputUnfocus}/>
            ...
            <InputTags label={$LL.mapEditor.settings.megaphone.inputs.rights()} options={tags ?? []} bind:value={rights} onFocus={onMapEditorInputFocus} onBlur={onMapEditorInputUnfocus}/>
