<script lang="ts">
    export let collisionGrid: number[][] = [];
    export let collisionGridHeight: number;
    ...
    {#each collisionGrid as row, rowIndex (rowIndex)}
        {#each row as _, columnIndex (columnIndex)}
            <div
                ...
                class={`tw-cursor-pointer tw-border-solid tw-border tw-border-red-400 ${
                    collisionGrid[rowIndex][columnIndex] === 1 ? "tw-bg-red-500" : ""
                } tw-bg-opacity-30`}
