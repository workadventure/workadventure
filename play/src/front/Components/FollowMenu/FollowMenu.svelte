...
{#if $followStateStore === "requesting" && $followRoleStore === "follower"}
    <div class="interact-menu text-center text-white w-96 absolute bottom-12 left-0 right-0 pointer-events-auto z-[150] right-0 left-0 m-auto bg-contrast/80 backdrop-blur rounded-lg overflow-hidden">
        ...
        <div class="flex flex-row justify-evenly bg-contrast p-4">
            <button type="button" class="btn btn-ghost btn-sm btn-light w-full justify-center mr-1" on:click|preventDefault={reset}
            >{$LL.follow.interactMenu.no()}
            </button>
            <button type="button" class="btn btn-secondary btn-sm w-full justify-center ml-1" on:click|preventDefault={acceptFollowRequest}
