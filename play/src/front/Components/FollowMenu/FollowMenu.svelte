...
{#if $followStateStore === "requesting" && $followRoleStore === "follower"}
    <div class="interact-menu text-center text-white w-[500px] h-[250px] absolute mt-8 left-0 right-0 pointer-events-auto z-[150] right-0 left-0 m-auto rounded-lg overflow-hidden margin-bottom">
        <div class="mt-12 text-lg bold">
            <div>
                <svg class="opacity-50 mb-4" width="23" height="23" viewBox="0 0 23 23" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="flex p-4 space-x-4 bg-contrast mt-12 pointer-events-auto mt-18">
            ...
            <button type="button" class="btn btn-secondary btn btn-secondary w-1/2 justify-center" on:click|preventDefault={acceptFollowRequest}
...
{#if $followStateStore === "active" || $followStateStore === "ending"}
    <div
        class="blue-dialog-box outline-light w-96 min-h-10 absolute bottom-16 mb-16 text-center m-auto right-0 left-0 z-[150] rounded-lg overflow-hidden text-white hover:animate-none transition-all pointer-events-auto"
    >

        {#if $followRoleStore === "follower"}
            <div class="m-1 px-8 py-4">{$LL.follow.interactStatus.following({ leader: name($followUsersStore[0]) })}</div>
            <div class="flex flex-row justify-evenly bg-contrast p-4">
                <button type="button" class="btn btn-sm btn-danger w-full justify-center" on:click|preventDefault={reset}>Stop following
                </button>
            </div>
        ...
        {:else if $followUsersStore.length === 1}
            <div class="m-1 px-8 py-4">{$LL.follow.interactStatus.followed.one({ follower: name($followUsersStore[0]) })}</div>
        ...
        {:else}
            ...
            <div class="flex flex-row justify-evenly bg-contrast p-4">
                <button type="button" class="btn btn-sm btn-danger w-full justify-center" on:click|preventDefault={reset}>Cancel
                </button>
            </div>
        {/if}
    </div>
