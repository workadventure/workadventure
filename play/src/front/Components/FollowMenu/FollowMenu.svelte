...
{#if $followStateStore === "requesting" && $followRoleStore === "follower"}
    <div class="interact-menu text-center text-white w-[500px] absolute bottom-12 left-0 right-0 pointer-events-auto z-[150] right-0 left-0 m-auto rounded-lg overflow-hidden margin-bottom">
        <div class="mb-6 text-lg bold">
            <div>
...
{#if $followStateStore === "active" || $followStateStore === "ending"}
    <div
        class="blue-dialog-box outline-light w-72 min-h-10 absolute bottom-16 text-center m-auto right-0 left-0 z-[150] bg-contrast/80 backdrop-blur rounded-lg overflow-hidden text-white hover:animate-none transition-all pointer-events-auto"
    >
        {#if $followRoleStore === "follower"}
