    ...
    >
        <div class="space-y-4">
            <h3 class="text-xl font-bold text-white">
                {$LL.onboarding.pictureInPicture.title()}
            </h3>
            <p class="text-sm text-white/90 mb-4">
                {$LL.onboarding.pictureInPicture.description()}
            </p>
            {#if $LL.onboarding.pictureInPicture.video()}
                <video autoplay muted loop class="w-full rounded-lg mb-4">
                    <source src={$LL.onboarding.pictureInPicture.video()} type="video/mp4" />
                </video>
            {/if}
            <p class="text-xs text-white/70 italic">
                {$LL.onboarding.pictureInPicture.hint()}
            </p>
            <button
                class="mt-4 px-4 py-2 bg-secondary hover:bg-secondary-600 text-white rounded-lg font-semibold transition-all"
                data-testid="onboarding-button-pictureInPicture-next"
                on:click={handleNext}
            >
                {$LL.onboarding.pictureInPicture.next()}
            </button>
        </div>
    </div>
</div>
