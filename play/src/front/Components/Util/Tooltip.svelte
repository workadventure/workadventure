...
<div bind:this={tooltipElement}  in:fly={{y: 40, duration: 150 }}>
    <div bind:this={textElement} class="fixed top-[70px] text-sm rounded w-40 px-4 py-2 text-white left-1/2 transform -translate-x-1/2 text-center before:content-[''] before:absolute before:w-full before:h-full before:z-1 before:left-0 before:top-0 before:rounded before:bg-contrast/80 before:backdrop-blur after:content-[''] after:absolute after:z-0 after:w-full after:bg-transparent after:h-full after:-top-4 after:-left-0 transition-all {toolTipPosition}">
        <img loading="eager" src="{tooltipArrow}" class="content-[''] absolute -top-1 left-0 right-0 m-auto w-2 h-1" style="visibility: hidden;" />
        <span class="relative">
            {text}
        </span>
    </div>
