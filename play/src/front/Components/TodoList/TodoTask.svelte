...
<div class="tw-flex tw-flex-col tw-text-left tw-p-2 hover:tw-border-white hover:tw-border hover:tw-border-solid">
    <p class="tw-text-lg tw-m-0 tw-p-0"
        class:tw-line-through={task.status === "completed"}
    >
        {task.title} {#if task.end}<span class="tw-text-sm">({task.end.toLocaleDateString()})</span>{/if}
    </p>
