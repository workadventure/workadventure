<script lang="ts">
    ...
    let subscribers = new Array<Unsubscriber | Subscription>();
    ...
    onDestroy(() => {
        subscribers.map((subscriber) => (subscriber instanceof Subscription ? subscriber.unsubscribe() : subscriber()));
    });
...
<div class="flex absolute w-full p-2 xl:p-4 pointer-events-none bp-menu z-[301] top-0 @container/actions">
    <div class="justify-start flex-1 pointer-events-auto w-32" transition:fly={{ delay: 500, y: -200, duration: 750 }}>
        ...
        <div class="flex relative transition-all duration-150 z-[2]" class:opacity-0={$chatVisibilityStore}>
            ...
            >
                <div
                    class="h-12 w-12 rounded group-hover/btn-message-circle:bg-white/10 aspect-square flex items-center justify-center transition-all"
                >
                    <MessageCircleIcon />
                </div>
                ...
                {#if $chatZoneLiveStore || $peerStore.size > 0}
                    <div>
                        <span
                            class="w-4 h-4 block rounded-full absolute -top-1 -left-1 animate-ping {$peerStore.size > 0
                                ? 'bg-success'
                                : 'bg-danger'}"
                        />
                        <span
            ...
            >
                <div
                    class="h-12 w-12 rounded group-hover/btn-users:bg-white/10 aspect-square flex items-center justify-center transition-all"
                    on:mouseenter={() => {
    ...
    <div
        id="main-action"
        class="@sm/main:justify-center justify-end flex-1 m-auto pointer-events-auto w-[200px] @container/main:"
    >
        <div class="flex justify-center relative space-x-2 xl:space-x-4">
            {#if !$silentStore}
                <div in:fly={{ delay: 750, y: -200, duration: 750 }}>
                    <div class="flex items-center">
                        ...
                        >
                            <div
                                class="h-12 w-12 rounded aspect-square flex items-center justify-center transition-all {$emoteMenuSubStore
                        ...
                        {#if $bottomActionBarVisibilityStore}
                            <!-- svelte-ignore a11y-click-events-have-key-events -->
                            <div
                                class="group/btn-layout bg-contrast/80 transition-all backdrop-blur p-2 pr-0 last:pr-2 first:rounded-l-lg last:rounded-r-lg  aspect-square"
                            >
                                <div
                                    class="h-12 w-12 rounded btn-layout/btn-more:bg-white/10 aspect-square flex items-center justify-center transition-all"
                            ...
                            <div
                                class="group/btn-follow bg-contrast/80 transition-all backdrop-blur p-2 pr-0 last:pr-2 first:rounded-l-lg last:rounded-r-lg  aspect-square"
                            >
                                <!-- svelte-ignore a11y-click-events-have-key-events -->
                                <div
                                    class="h-12 w-12 rounded group-hover/btn-follow:bg-white/10 aspect-square flex items-center justify-center transition-all {$followStateStore ===
                                    'active'
                                        ? 'bg-secondary'
                                        : ''}"
                            ...
                            <!-- svelte-ignore a11y-click-events-have-key-events -->
                            <div
                                class="group/btn-lock relative bg-contrast/80 backdrop-blur p-2 pr-0 last:pr-2 first:rounded-l-lg last:rounded-r-lg aspect-square"
                            ...
                            >
                                <div
                                    class="h-12 w-12 p-1 m-0 rounded group-[.disabled]/btn-lock:bg-secondary hover:bg-white/10 flex items-center justify-center transition-all"
                                >
            ...
            <div in:fly={{ delay: 1000, y: -200, duration: 750 }}>
                ...
                <div class="group/hardware flex items-center relative">
                    ...
                    {/if}-->
                    <div
                        class="absolute h-3 w-7 rounded-b bg-contrast/80 backdrop-blur left-0 right-0 m-auto p-1 z-10 opacity-0 transition-all -bottom-3 {cameraActive
                            ? 'opacity-100'
                    ...
                    >
                        ...
                        >
                            <ChevronUpIcon
                                height="h-4"
                                width="w-4"
                                classList="aspect-ratio transition-all {cameraActive ? '' : 'rotate-180'}"
                                strokeWidth="2"
                            />
                        </div>
                    </div>
                    ...
                    {#if $myCameraStore && !$silentStore}
                        <!-- svelte-ignore a11y-click-events-have-key-events -->
                        <div
                            class="group/btn-cam relative bg-contrast/80 backdrop-blur p-2 pr-0 last:pr-2 first:rounded-l-lg last:rounded-r-lg aspect-square"
                    ...
                    {#if $bottomActionBarVisibilityStore}
                        <div
                            class="group/btn-screen-share relative bg-contrast/80 backdrop-blur p-2 pr-0 last:pr-2 first:rounded-l-lg last:rounded-r-lg aspect-square"
                        ...
                        >
                            <div
                                class="h-12 w-12 p-1 m-0 rounded group-[.disabled]/btn-screen-share:bg-secondary hover:bg-white/10 flex items-center justify-center transition-all {$requestedScreenSharingState &&
                                !$silentStore
        ...
        <div class="flex justify-end space-x-2 xl:space-x-4">
            ...
            {#if $inviteUserActivated}
                ...
                <div id="action-invite">
                    <div class="flex items-center">
                        <div
                            class="bg-contrast/80 backdrop-blur p-2 pr-0 last:pr-2 first:rounded-l-lg last:rounded-r-lg flex"
                        >
                            ...
                            {#if $inviteUserActivated}
                                <button
                                    ...
                                    class="btn rounded h-12 select-none !px-4 {!$userIsConnected && ENABLE_OPENID
                                        ? 'btn-ghost btn-light'
                                        : 'btn-secondary'}"
                                >
                                    {$LL.menu.sub.invite()}
            ...
            {#if $mapEditorActivated || $userHasAccessToBackOfficeStore}
                <div id="action-admin" class="items-center relative">
                    ...
                    <div
                        class="group bg-contrast/80 backdrop-blur rounded-lg h-16 p-2"
                    ...
                    >
                        ...
                        >
                            <AdminPanIcon />
                            <div class="pr-2">
                                <div class="font-bold text-white leading-3 whitespace-nowrap select-none">
                                    Admin<!-- trad -->
                                </div>
                            </div>
                            <ChevronDownIcon
                                strokeWidth="2"
                                classList="h-4 w-4 aspect-ratio transition-all opacity-50 {adminMenuIsDropped
                                    ? 'rotate-180'
                                    : ''}"
                                height="16px"
                                width="16px"
                    ...
                    {#if adminMenuIsDropped}
                        <div
                            class="absolute mt-2 top-16 right-0 bg-contrast/80 backdrop-blur rounded-lg py-2 w-56 right-0 text-white before:content-[''] before:absolute before:w-0 before:h-0 before:-top-[14px] before:right-6 before:border-solid before:border-8 before:border-solid before:border-transparent before:border-b-contrast/80 transition-all"
                        ...
                        >
                            <ul class="p-0 m-0">
                                {#if $mapEditorActivated}
                                    ...
                                    >
                                        <div
                                            class="group-hover:mr-2 transition-all w-6 h-6 aspect-ratio mr-3 text-center"
                                        >
                                            <svg
                                                width="20"
                                                height="20"
            ...
            <div id="action-user" class="flex items-center relative min-w-40">
                ...
                >
                    ...
                    >
                        ...
                        <div class="grow flex flex-col justify-start text-left pr-2">
                            ...
                            <div class="text-xxs bold whitespace-nowrap select-none flex items-center">
                                {#if $availabilityStatusStore === 1}
                                    <div class="aspect-ratio h-2 w-2 bg-success rounded-full mr-2" />
                                    <div class="text-success">Online</div>
                                {/if}
                                {#if $availabilityStatusStore === 2}
                                    <div class="aspect-ratio h-2 w-2 bg-warning rounded-full mr-2" />
                                    <div class="text-warning">Away</div>
                                {/if}
                                {#if $availabilityStatusStore === 3}
                                    <div class="aspect-ratio h-2 w-2 bg-danger rounded-full mr-2" />
                        ...
                        <div>
                            <ChevronDownIcon
                                strokeWidth="2"
                                classList="transition-all opacity-50 {adminMenuIsDropped ? 'rotate-180' : ''}"
                                height="h-4"
                                width="w-4"
                            />
                        </div>
                ...
                {#if profileMenuIsDropped}
                    <div
                        class="absolute mt-2 top-16 bg-contrast/80 backdrop-blur rounded-lg py-2 w-56 right-0 text-white before:content-[''] before:absolute before:w-0 before:h-0 before:-top-[14px] before:right-6 before:border-solid before:border-8 before:border-solid before:border-transparent before:border-b-contrast/80 transition-all"
                        transition:fly={{ y: 40, duration: 150 }}
                    >
                        <div class="p-0 m-0 list-none">
                            ...
                            >
                                <div class="aspect-ratio h-2 w-2 bg-success rounded-full ml-2 mr-3" />
                                <div class="mr-3 grow text-left {$availabilityStatusStore === 1 ? '' : 'opacity-50'}">
                                    Online<!-- trad -->
                                </div>
                                {#if $availabilityStatusStore === 1}
                                    <div class="">
                            ...
                            <button
                                ...
                                on:click={() => openEditNameScene()}
                            >
                                <div class="aspect-ratio h-2 w-2 bg-warning rounded-full ml-2 mr-3" />
                                <div class="mr-3 grow text-left {$availabilityStatusStore === 2 ? '' : 'opacity-50'}">
                                    Away<!-- trad -->
                                </div>
                                {#if $availabilityStatusStore === 2}
                                    <div class="">
                            ...
                            <button
                                ...
                                on:click={() => openEditNameScene()}
                            >
                                <div class="aspect-ratio h-2 w-2 bg-danger rounded-full ml-2 mr-3" />
                                <div class="mr-3 grow text-left {$availabilityStatusStore === 3 ? '' : 'opacity-50'}">
                                    Do not disturb<!-- trad -->
                                </div>
                                {#if $availabilityStatusStore === 3}
                                    <div class="">
                            ...
                            <button
                                ...
                                on:click={() => openEditNameScene()}
                            >
                                <div class="aspect-ratio h-2 w-2 bg-neutral rounded-full ml-2 mr-3" />
                                <div class="mr-3 grow text-left {$availabilityStatusStore === 4 ? '' : 'opacity-50'}">
                                    Offline<!-- trad -->
                                </div>
            ...
            <div
                class="group/btn-burger relative bg-contrast/80 backdrop-blur p-2 pr-0 last:pr-2 rounded-l-lg rounded-r-lg aspect-square"
            >
                <!-- svelte-ignore a11y-click-events-have-key-events -->
                <div
...
{#if burgerOpen}
    <div
        class="w-52 bg-contrast/80 absolute right-2 top-20 z-[1000] py-4 rounded-lg text-right text-white no-underline pointer-events-auto"
    >
            ...
            <div class="px-4">
                ...
                {#if $inviteUserActivated}
                    <button
                        ...
                        on:click={() => analyticsClient.openInvite()}
                        on:click={() => showMenuItem(SubMenusInterface.invite)}
                        class="btn btn-sm !px-4 w-full justify-center {!$userIsConnected && ENABLE_OPENID
                            ? 'btn-ghost btn-light'
                            : 'btn-secondary'}"
                    >
                        {$LL.menu.sub.invite()}
            ...
            {#each $addActionButtonActionBarEvent as button, index (index)}
                <div class="px-4">
                    ...
                    <div
                        ...
                        on:dragstart|preventDefault={noDrag}
                        on:click={() =>
                            analyticsClient.clickOnCustomButton(button.id, undefined, button.toolTip, button.imageSrc)}
                        on:click={() => {
                            buttonActionBarTrigger(button.id);
                        }}
                        class="flex items-center justify-center btn btn-ghost btn-sm btn-light rounded select-none ml-2 !px-4 "
                    >
                        <img draggable="false" src={button.imageSrc} alt={button.toolTip} class="h-6 mr-4" />
                        {button.toolTip}
                        <!-- src="./static/images/Workadventure.gif"   src={button.imageSrc}   -->
                    </div>
                </div>
