    ...
    {#if !canShare}
        <section class="share-url not-mobile">
            ...
            <div class="flex items-center relative m-4">
                <input type="text" readonly id="input-share-link" class="grow h-12 text border-white bg-contrast rounded border border-solid border-white/20" value={location.toString()} />
                <button type="button" class="flex items-center btn btn-secondary btn-sm absolute right-2 transition-all w-32 text-center justify-center {linkCopied ? 'btn-success' : 'btn-secondary'}" on:click={() => linkCopied = !linkCopied} on:click={copyLink}>
                    <span hidden={!linkCopied}>
                        <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-check" width="16" height="16" viewBox="0 0 24 24" stroke-width="1.5" stroke="#ffffff" fill="none" stroke-linecap="round" stroke-linejoin="round">
                          <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
