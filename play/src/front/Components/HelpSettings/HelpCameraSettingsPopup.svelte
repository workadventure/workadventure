...
>
    ...
    <section class="tw-flex tw-row tw-justify-center">
        <button class="light" on:pointerdown|preventDefault={refresh}>{$LL.camera.help.refresh()}</button>
        <button type="submit" class="outline" on:pointerdown|preventDefault={close}>{$LL.camera.help.continue()}</button
        >
    </section>
</form>
