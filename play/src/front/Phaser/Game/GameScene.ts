...
export class GameScene extends DirtyScene {
    ...
    private calculateViewport(margin = 300): ViewportInterface | null {
        ...
        const baseLeft = Math.max(0, camera.scrollX - margin);
        const baseTop = Math.max(0, camera.scrollY - margin);
        const baseRight = camera.scrollX + camera.width + margin;
        const baseBottom = camera.scrollY + camera.height + margin;
