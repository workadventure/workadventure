<script lang="ts">
        ...
        navigator.clipboard
            ...
            .then(() => {
                ...
                if (target) {
                    ...
                    target.innerHTML = originalText.replace($LL.chat.copy(), $LL.chat.copied());
                    setTimeout(() => {
    ...
    function openCowebsite() {
        if (embedLink) void openCoWebSite(embedLink, true, "allowfullscreen");
        if (message.links)
            message.links.forEach((link) => void openCoWebSite(link.url, true, "allowfullscreen"));
    }
            ...
            {#if !$error && !$deletedMessagesStore.has(message.id)}
                ...
                >
                    ...
                    {#if ($me && $me.isAdmin) || isMe}
                        ...
                        >
                            <Trash2Icon size="17" />
                            <div class="caption">{$LL.chat.delete()}</div>
                        </div>
                    {/if}
                </div>
            {/if}
                    ...
                    <div class="wa-message-body">
                        <p class="tw-italic">
                            {#if JID.toBare(message.jid) === $deletedMessagesStore.get(message.id)}
                                ...
                                {:else}
                                    {$LL.chat.messageDeleted()}{message.name}.
                                {/if}
                            {:else}
                                {$LL.chat.messageDeleted()}{$LL.chat.anAdmin()}.
                            {/if}
                        </p>
                    </div>
