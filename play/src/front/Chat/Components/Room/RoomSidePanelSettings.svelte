<script lang="ts">
    ...
    export let room: ChatRoom & ChatRoomMembershipManagement & ChatRoomModeration & ChatRoomSettingsManagement;

    const emptyPrivacyState = readable<ChatRoomPrivacyState>({});
    const roomName = room.name;
    const roomTopic = room.topic;
    const roomPermissions = room.permissionsState;
    ...
    let editableName = "";
    let editableTopic = "";
    let editableAccess: ChatRoomSettingsAccess = "invite";
    let editableHistoryVisibility: historyVisibility = "joined";
    let editablePermissions: ChatRoomPermissionsState;

    $: roomType = room.type;
    $: isEncrypted = room.isEncrypted;
    $: privacyState = room.privacyState ?? emptyPrivacyState;
    $: restrictedRoomId = $privacyState.restrictedRoomId;
    $: canUseRestrictedAccess = restrictedRoomId !== undefined;
    $: hasAnyEditableField = $canEditName || $canEditTopic || $canEditAccess || $canEditHistory || $canEditPermissions;
                        ...
                        {#each permissionRows as permissionRow (permissionRow.key)}
                            <label class="flex items-center justify-between gap-3 text-white/80">
                                ...
                                <select
                                    ...
                                    on:change={markPermissionsDirty}
                                >
