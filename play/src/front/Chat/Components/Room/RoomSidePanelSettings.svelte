<script lang="ts">
    ...
    const defaultEditablePermissions = Object.fromEntries(
        permissionRows.map(({ key }) => [key, ChatPermissionLevel.USER]),
    ) as ChatRoomPermissionsState;
    ...
    let editableAccess = $state<ChatRoomSettingsAccess>("invite");
    let editableHistoryVisibility = $state<historyVisibility>("joined");
    let editablePermissions = $state<ChatRoomPermissionsState>({ ...defaultEditablePermissions });
