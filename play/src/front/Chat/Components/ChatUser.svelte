    ...
    >
        <h1 class={`tw-text-sm tw-font-bold tw-mb-0  tw-cursor-default`}>
            ...
            {#if ENABLE_OPENID}
                ...
                {:else}
                    <span class="tw-text-pop-red" title={$LL.chat.role.visitor()}>
                        <UserXIcon size="13" />
                    </span>
                    -->
                {/if}
                ...
                {#if $me && $me.isAdmin}
                    ...
                    {#if user.isAdmin}
                        <span
                            ...
                            ><ShieldOffIcon size="13" /> {$LL.chat.rankDown()} (coming soon)</span
                        >
                    {:else}
                        <span
                            class="rank-up wa-dropdown-item tw-text-warning"
                            on:click|stopPropagation={() => mucRoom.sendRankUp(user.jid)}
                            ><ShieldIcon size="13" /> {$LL.chat.rankUp()} (coming soon)</span
                        >
                    {/if}
                {/if}
