<script lang="ts">
    ...
    $: usersByMaps = $presenceStore
        ...
        .reduce((reduced, user: Writable<User>) => {
            ...
            if ((group === "disconnected" && connectionManager.currentRoom?.enableChatDisconnectedList) || group !== "disconnected") {
...
{#if $loadingSubscribersStore}
    <Loader text={$LL.chat.loadingUsers()} height="tw-h-40" />
{:else}
