<script lang="ts">
    ...
    onMount(() => {
        ...
        picker = new EmojiButton({
            ...
            showPreview: false,
            i18n: {
                search: $LL.chat.emoji.search(),
                categories: {
                    recents: $LL.chat.emoji.categories.recents(),
                    smileys: $LL.chat.emoji.categories.smileys(),
                    people: $LL.chat.emoji.categories.people(),
                    animals: $LL.chat.emoji.categories.animals(),
                    food: $LL.chat.emoji.categories.food(),
                    activities: $LL.chat.emoji.categories.activities(),
                    travel: $LL.chat.emoji.categories.travel(),
                    objects: $LL.chat.emoji.categories.objects(),
                    symbols: $LL.chat.emoji.categories.symbols(),
                    flags: $LL.chat.emoji.categories.flags(),
                    custom: $LL.chat.emoji.categories.custom(),
                },
                notFound: $LL.chat.emoji.notFound(),
            },
        });
            ...
            case "Klaxoon":
                try {
                    ...
                    if (err instanceof KlaxoonException.KlaxoonException) {
                        app.error = $LL.chat.form.application.klaxoon.error();
                    } else {
                        app.error = $LL.chat.form.application.weblink.error();
                    }
                    app.link = undefined;
                }
                break;
            case "Youtube":
                try {
                    ...
                    if (err instanceof GoogleWorkSpaceException.YoutubeException) {
                        app.error = $LL.chat.form.application.youtube.error();
                    } else {
                        app.error = $LL.chat.form.application.weblink.error();
                    }
                    app.link = undefined;
            ...
            case "Google Docs":
                try {
                    ...
                    if (err instanceof GoogleWorkSpaceException.GoogleDocsException) {
                        app.error = $LL.chat.form.application.googleDocs.error();
                    } else {
                        app.error = $LL.chat.form.application.weblink.error();
                    }
                    app.link = undefined;
            ...
            case "Google Sheets":
                try {
                    ...
                    if (err instanceof GoogleWorkSpaceException.GoogleSheetsException) {
                        app.error = $LL.chat.form.application.googleSheets.error();
                    } else {
                        app.error = $LL.chat.form.application.weblink.error();
                    }
                    app.link = undefined;
            ...
            case "Google Slides":
                try {
                    ...
                    if (err instanceof GoogleWorkSpaceException.GoogleSlidesException) {
                        app.error = $LL.chat.form.application.googleSlides.error();
                    } else {
                        app.error = $LL.chat.form.application.weblink.error();
                    }
                    app.link = undefined;
            ...
            case "Eraser":
                try {
                    ...
                    if (err instanceof EraserException.EraserLinkException) {
                        app.error = $LL.chat.form.application.eraser.error();
                    } else {
                        app.error = $LL.chat.form.application.weblink.error();
                    }
                    app.link = undefined;
...
<div
    id="activeTimeline"
    class="tw-flex tw-flex-col tw-h-full tw-min-h-full tw-over tw-w-ful"
        ...
        <div class="tw-flex tw-flex-col tw-flex-auto tw-w-full">
            ...
            >
                <button class="wa-action" type="button" on:click|stopPropagation={reInitialize}
                    ><RefreshCwIcon size="13" class="tw-mr-2" /> {$LL.chat.reinit()}
                </button>
            </div>
        </div>
    </div>
        ...
        <form on:submit|preventDefault={saveMessage} class="tw-flex tw-flex-col">
            ...
            <div class="tw-w-full tw-px-2 tw-pb-2 tw-flex tw-flex-row tw-justify-center tw-items-center">
                ...
                <div
                    ...
                    data-placeholder={$LL.chat.enterText()}
                ...
                <div class="tw-w-full tw-p-2">
                    <div class="tw-flex tw-items-center tw-relative">
                        <textarea
                            ...
                            placeholder={$LL.chat.form.placeholder()}
                            on:keydown={handlerKeyDown}
                            on:input={writing}
                            on:focus={onFocus}
                            on:blur={onBlur}
