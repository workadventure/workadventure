...
>
    {#if $selectedRoom === undefined || displayTwoColumnLayout}
        ...
        >
            <div class="tw-p-2 tw-flex tw-items-center tw-absolute tw-w-full">
            ...
            <div class="tw-relative tw-pt-[72px] {($isEncryptionRequiredAndNotSet === true && $isGuest === false) ? ' tw-h-[calc(100%-2rem)]' : 'tw-h-full'}">
                ...
                <div class="tw-px-2 tw-py-3 tw-border tw-border-solid tw-border-x-0 tw-border-t tw-border-b-0 tw-border-white/10">
                    <div
                            class="tw-group tw-relative tw-px-3 tw-rounded-md tw-h-11 tw-w-full tw-flex tw-space-x-2 tw-items-center {$proximityHasUnreadMessages ? 'hover:tw-bg-contrast-200/20 tw-bg-contrast-200/10' : 'hover:tw-bg-contrast-200/10'}"
                    >
                        <button
                        ...
                        >
                            <div class="tw-relative">
                                <div class="tw-rounded-full tw-bg-white/10 tw-h-7 tw-w-7 tw-border tw-border-solid tw-text-white tw-flex tw-items-center tw-justify-center tw-p-[1px] tw-relative {$proximityHasUnreadMessages ? 'tw-border-white' : 'tw-border-white/70'}">

                                    <div class="tw-absolute tw-overflow-hidden tw-w-full tw-h-full tw-rounded-full">
                                        <div class="tw-translate-y-[3px] -tw-translate-x-[3px] group-hover:tw-translate-y-[0] tw-transition-all">
                                            <WokaFromUserId userId={-1} customWidth="32px" customHeight="32px" placeholderSrc="" />
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="tw-cursor-default tw-text-sm tw-grow tw-text-left tw-pl-1 {$proximityHasUnreadMessages ? 'tw-text-white tw-font-bold' : 'tw-text-white/75'}">
                                {$LL.chat.proximity()}
                            </div>
                ...
                {#if $chatConnectionStatus === "ONLINE"}
                    ...
                    {#if filteredRoomInvitations.length > 0}
                        ...
                        >
                            ...
                            <div class="tw-transition-all group-hover:tw-bg-white/10 tw-p-1 tw-rounded-lg tw-aspect-square tw-flex tw-items-center tw-justify-center">
            ...
            <div class="tw-sticky tw-bottom-0 tw-w-full tw-backdrop-blur-md tw-mt-3">
                <button
                    ...
                    class="tw-text-white tw-flex tw-gap-2 tw-justify-center tw-w-full tw-bg-white/20 hover:tw-bg-neutral hover:tw-brightness-100 tw-m-0 tw-rounded-none tw-py-2 tw-px-3"
                >
    ...
    {:else if $selectedRoom === undefined && sideBarWidth >= CHAT_LAYOUT_LIMIT}
        <div class="tw-flex tw-flex-col tw-flex-1 tw-pl-4">
            ...
            <p class="tw-self-center tw-text-md tw-text-gray-500">{$LL.chat.nothingToDisplay()}</p>XXXX
        </div>
    {/if}
</div>
