<script lang="ts">
    ...
    function handlerDeleteUploadedFile(file: FileExt | UploadedFile) {
        ...
        filesUploadStore.update((list : Map<string, UploadedFile | FileExt>) => {
        ...
        switch (app.name) {
            ...
            case "Youtube":
                try {
                    ...
                    console.info($LL.chat.form.application.youtube.error(), e);
        ...
        <div class="replyMessage" on:click={() => selectedMessageToReply.set(null)}>
            ...
            >
                <span class={`tw-text-lighter-purple ${isMe($selectedMessageToReply.name) ? "tw-ml-2" : "tw-mr-2"}`}
                    >{#if isMe($selectedMessageToReply.name)}{$LL.chat.me()}{:else}{$selectedMessageToReply.name}{/if}</span
                >
                        ...
                        >
                            ...
                            {#if fileUploaded.errorMessage === "not-logged"}
                                <div
                                    ...
                                    on:click|preventDefault|stopPropagation={() => sendLogin()}
                                >
            ...
            <div class="tw-w-full tw-px-2 tw-pb-2 tw-flex tw-flex-row tw-justify-center tw-items-center">
                ...
                <div class="actions tw-px-2 tw-py-2">
                    <div class="tw-flex tw-items-center tw-space-x-1">
                        ...
                        {#if connectionManager.currentRoom?.enableChatUpload}
                            <input
                                type="file"
                                id="file"
                                name="file"
                        ...
                        {:else}
                            <button
                                ...
                                on:click|preventDefault|stopPropagation={() =>
                                    (informationMessage = $LL.chat.disabledByAdmin())}
                            >
