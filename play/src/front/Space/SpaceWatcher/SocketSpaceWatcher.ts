...
export class StreamSpaceWatcher {
    constructor(
        ...
        chatConnectionPromise: Promise<ChatConnectionInterface>,
        proximityRoom: ChatConnectionInterface | undefined = get(proximityRoomConnection)
    ) {
        this.socket.addEventListener("message", (messageEvent: MessageEvent) => {
            (async () => {
                const chatConnection = await chatConnectionPromise;
                ...
                for (const subMessageWrapper of message.batchMessage.payload) {
                    ...
                    switch (subMessage.$case) {
                        ...
                        case SpaceEvent.UpdateSpaceUser: {
                            if (
                                !subMessage.updateSpaceUserMessage.user ||
                                !subMessage.updateSpaceUserMessage.filterName
                            )
                                return;
                        ...
                        case SpaceEvent.RemoveSpaceUser: {
                            if (
                                !subMessage.removeSpaceUserMessage.userId ||
                                !subMessage.removeSpaceUserMessage.filterName
                            )
                                return;
                            this.spaceProvider
...
export class StreamSpaceWatcherSingleton {
    ...
    static getInstance(socket: WebSocket, chatConnectionPromise: Promise<ChatConnectionInterface>): StreamSpaceWatcher {
